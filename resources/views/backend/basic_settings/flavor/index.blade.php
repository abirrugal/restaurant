...
>
  <i class="fa fa-edit"> </i> Create a Flavor
</button>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header action_button text-white">
        <h6 class="modal-title" id="staticBackdropLabel"><i class="fa fa-edit"> </i> Create New Flavor</h6>
      ...
      <div class="modal-body">
        ...
        <form action="{{route('admin.flavor.create')}}" method="post" enctype="multipart/form-data">
          @csrf
...
<div class="col">
<div class="mb-3">
  <label for="number" class="form-label text-black h6">Flavor Name</label>  
  <input type="text" name="name" value="{{old('name')}}" class="form-control" id="name" aria-describedby="PriceHelp" placeholder="Flavor Name" required>
  ...
  <select class="form-select custom-select" aria-label="Default select example" name="status" id="status" required>
    ...
    <option value="Inactive" {{old('inactive')=== 'inactive'? 'selected' : '' }}>Inactive</option>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="add_flavor()">Create Flavor</button>
      </div>
 </form>
      </div>
...
<input type="hidden" id ="id">
  ...
  <div class="mb-3">
  <label for="slider_image" class="form-label text-black h6">Status</label>
  <select class="form-select custom-select" aria-label="Default select example" name="status" id="editstatus" >
...
<script type="text/javascript">
  ...
  $.ajax({
      type:"GET",
      dataType: 'json',
      url:"{{asset('/')}}admin/flavor/alldata",
...
function editdata(id){
  $.ajax({
      ...
      url: "{{asset('/')}}admin/flavor/edit/"+id,
      success:function(response){
...
return del;
  ...
  function changests(id){
    $.ajax({
      ...
      url: "{{asset('/')}}admin/flavor/"+id+"/change_sts",
      success:function(response){
