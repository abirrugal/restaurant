...
>
  <i class="fa fa-edit"> </i> Create a Product Material Settings
</button>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header action_button text-white">
        <h6 class="modal-title" id="staticBackdropLabel"><i class="fa fa-edit"> </i> Create New Product Material Settings</h6>
      ...
      <div class="modal-body">
        ...
        <form action="{{route('admin.product_mat_setting.create')}}" method="post" enctype="multipart/form-data">
          @csrf
  ...
  <select class="form-select custom-select" aria-label="Default select example" name="status" id="stock_status" required>
    ...
    <option value="Inactive" {{old('inactive')=== 'inactive'? 'selected' : '' }}>Inactive</option>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary">Create Product Material Settings</button>
      </div>
 </form>
      </div>
...
<table class="table table-bordered rounded">
  <thead class="rounded">
    <tr class="text-center rounded table_title_color">
      ...
      <th scope="col">Product Materials</th>
...
@endif
      ...
      <td class="text-center">
        <a class="text-center" href="{{route('admin.product_mat_setting.change_sts', $info->id)}}"><i class="fas fa-exchange-alt d-block"></i></a>
      </td>
      <td>
  ...
  <a href="{{route('admin.product_mat_setting.edit', $info->id)}}">
...
<i class="far fa-edit"></i> 
 ...
 {{-- Delete  --}}
        ...
        @if (auth()->user()->role_as !== 'creator')
         ...
         <form class="d-inline " onclick="return confirm('Sure to delete product ?')"  action="{{route('admin.product_mat_setting.delete', $info->id)}}" method="POST">
          @csrf
          @method('Delete')
...
<script>
  ...
  function editdata(id){
    $.ajax({
        ...
        url: "{{asset('/')}}admin/product_mat_setting/edit/"+id,
  ...
  function updatedata(){
   ...
   var name = $('#epro_name').val();
    ...
    $.ajax({
        ...
        url: "{{asset('/')}}admin/product_mat_setting/"+id,
        success:function(response){
