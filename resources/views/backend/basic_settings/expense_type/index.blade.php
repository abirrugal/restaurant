...
>
  <i class="fa fa-edit"> </i> Create a Expense Type
</button>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header action_button text-white">
        <h6 class="modal-title" id="staticBackdropLabel"><i class="fa fa-edit"> </i> Create New Expense Type</h6>
      ...
      <div class="modal-body">
        ...
        <form action="{{route('admin.expense_type.create')}}" method="post" enctype="multipart/form-data">
          @csrf
...
<div class="col">
<div class="mb-3">
  <label for="number" class="form-label text-black h6">Expense Type Name</label>  
  <input type="text" name="name" value="{{old('name')}}" class="form-control" id="name" aria-describedby="PriceHelp" placeholder="Expense Type Name" required>
...
<div class="form-floating">
  ...
  <label for="floatingTextarea2">Expense Details</label>
</div>
...
@endif
      ...
      <td class="text-center">
        <a class="text-center" href="{{route('admin.expense_type.change_sts', $info->id)}}"><i class="fas fa-exchange-alt d-block"></i></a>
      </td>
      <td>
...
<div class="modal fade" id="edit" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header action_button text-white">
        <h6 class="modal-title" id="editLabel"><i class="fa fa-edit"> </i> Edit Expense Type</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      ...
      <div class="modal-body">
        ...
        <form action="{{route('admin.expense_type.update', $info->id)}}" method="post" enctype="multipart/form-data">
          @method('put')
          @csrf
...
<div class="col">
<div class="mb-3">
  <label for="number" class="form-label text-black h6 text-left">Expense Type Name</label>  
  <input type="text" name="name" value="{{$info->name}}" class="form-control" id="ename" aria-describedby="PriceHelp" placeholder="Expense Type Name">
...
<div class="form-floating">
  ...
  <select class="form-select custom-select" aria-label="Default select example" name="status" id="estatus" >
    ...
    <option value="Inactive" {{$info->status === 'Inactive'? 'selected' : '' }}>Inactive</option>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick="updatedata()">Edit Expense Type</button>
      </div>
 </form>
      </div>
...
@if (auth()->user()->role_as !== 'creator')
         ...
         <form class="d-inline " onclick="return confirm('Sure to delete product ?')"  action="{{route('admin.expense_type.delete',$info->id)}}" method="POST">
          @csrf
          @method('Delete')
...
<script>
  ...
  function editdata(id){
    $.ajax({
        ...
        url: "{{asset('/')}}admin/expense_type/edit/"+id,
  ...
  function updatedata(){
    ...
    $.ajax({
        ...
        url: "{{asset('/')}}admin/expense_type/"+id,
        success:function(response){
