...
>
  <i class="fa fa-edit"> </i> Create a Chocolate Flavor
</button>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header action_button text-white">
        <h6 class="modal-title" id="staticBackdropLabel"><i class="fa fa-edit"> </i> Create New Chocolate Flavor</h6>
      ...
      <div class="modal-body">
        ...
        <form action="{{route('admin.chocolate_flavor.create')}}" method="post" enctype="multipart/form-data">
          @csrf
...
<div class="col">
<div class="mb-3">
  <label for="number" class="form-label text-black h6">Chocolate Flavor Name</label>  
  <input type="text" name="name" value="{{old('name')}}" class="form-control" id="name" aria-describedby="PriceHelp" placeholder="Chocolate Flavor Name" required>
  ...
  <select class="form-select custom-select" aria-label="Default select example" name="status" id="status" required>
    ...
    <option value="Inactive" {{old('inactive')=== 'inactive'? 'selected' : '' }}>Inactive</option>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="add_flavor()">Create Chocolate Flavor</button>
      </div>
 </form>
      </div>
...
<table class="table table-bordered rounded">
  <thead class="rounded">
    <tr class="text-center rounded table_title_color">
      ...
      <th scope="col">Chocolate Flavor Name</th>
...
<script type="text/javascript">
  ...
  $.ajax({
      type:"GET",
      dataType: 'json',
      url:"{{asset('/')}}admin/chocolate_flavor/alldata",
...
$.ajax({
  ...
  url: "{{asset('/')}}admin/chocolate_flavor/create",
  success:function(response){
$('#name').val('');
$('#status').val('');
...
function editdata(id){
  $.ajax({
      ...
      url: "{{asset('/')}}admin/chocolate_flavor/edit/"+id,
      success:function(response){
...
function updatedata(){
  ...
  $.ajax({
      ...
      url: "{{asset('/')}}admin/chocolate_flavor/"+id,
      success:function(response){
...
return del;
  ...
  function changests(id){
    $.ajax({
      ...
      url: "{{asset('/')}}admin/chocolate_flavor/"+id+"/change_sts",
      success:function(response){
